...
<%
    ...
    int room_n = Integer.parseInt(request.getParameter("room_number"));
    ...
    stb.append("select * from reservation r1 right join room r2 on r1.room_num = r2.room_num where r2.room_num = \"").append(room_n).append("\"");
    String sql = stb.toString();
    stmt = conn.createStatement();
    rs = stmt.executeQuery(sql);
    rs.next();
    int roomnum = rs.getInt("r2.room_num");
%>
<!DOCTYPE html>
<html>
        ...
        <div class="tabs">
            <div id="subtitleCon">
                <div class="subTitle" style="padding-top: 10px; height: 45px; width:100%;">
                    <ul>
                        <li style="float: left; width: 15%; white-space: nowrap; overflow: hidden; 
						text-overflow: ellipsis; font-size: 25px ;background-color:#ff7609; border-radius:10px;
						 color:#f5f5f5; margin-left:5%; margin-top:1%;margin-bottom:2%" id="CurrentMenuTitle">방 세부정보
                        </li> <!--여기부터 작업-->
            ...
            <div class="form_table" style="margin-top: 0px; float: left; width: 100%; position: relative;">
                <table class="tbl_type" id= "reser" border="1" cellspacing="0" style="width:100%">
                    <colgroup>
                        <col style="width:15%">
                        <col style="width:20%">
                        <col style="width:15%">
                    ...
                    <tbody>
                    <% 
                        String city= rs.getString("r2.city");
                        String gu=rs.getString("r2.gu");
                        String ro=rs.getString("r2.ro");
                        String add=rs.getString("r2.d_address");
                        String add2 = rs.getString("r2.detail_add");
                        String juso = city + " " + gu + " " + ro + " " + add + " " + add2;
                        String drk =null;
                        String smk = null;
                        if(rs.getInt("r2.drink") == 1){
                            drk = "가능";
                        }else{
                            drk="불가능";
                        }
                        if(rs.getInt("r2.smoke") ==1){
                            smk ="가능";
                        }else{ smk="불가능"; }
                    %>
                        ...
                        <tr>
                            ...
                            <td colspan="5" style="text-align: left;padding:1% "><%=rs.getInt("r2.room_num")%>
                            </td>
                        </tr>
                        <tr>
                            <th style="text-align: left;color:#333;text-align: center;background-color:#fef5ef;"
                            ...
                            <td style="text-align: left;padding:1%"><%=rs.getInt("r2.user_num")%></td>
                            <th style=" text-align: left;color:#333;text-align: center;background-color:#fef5ef;">방 위치</th>
                            <td style="width: 100%; text-align: left;padding:1%"><%=juso%></td>
                        </tr>
                            ...
                            <tr style="height:400px">
                                ...
                                <td height="200" colspan="5" style="text-align: left;padding:1%">
                                    <p><방 정보></p>
                                    <p>시작 시간 : <%=rs.getTime("r2.start_time")%></p>
                                    <p>종료 시간 : <%=rs.getTime("r2.end_time")%></p>
                                    <p>흡연 가능 여부 : <%=smk%></p>
                                    <p>음주 가능 여부 : <%=drk%></p>
                                    <br>
                                    ...
                                    <p><예약자 정보></p>
                                    <% while (true) { %>
                                    <p>예약 번호 : <%=rs.getInt("r1.reser_num")%></p>
                                    <p>예약자 번호 : <%=rs.getInt("r1.user_num")%></p>
                                    <p>예약 시작 시간 : <%=rs.getTime("r1.start_time")%></p>
                                    <p>예약 종료 시간 : <%=rs.getTime("r1.end_time")%></p>
                                    <br>
                                    <%if(!rs.next()) {
                                        break;
                                    }%>
            ...
            <div style="float: left; width: 100%; text-align: center; margin-top: 3%; margin-bottom:15px;">
                ...
                <!--<input type ="hidden" id ="r2_room_number" value="0">-->
                <button style="width: 10%;background-color: #ffd7b4;border: solid 2px #ff895c69;border-radius: 10px;">
                    <a style="color: black;" href="delete_room.jsp?room_number=<%=roomnum%>">등록 삭제</a>
                </button>
            <!--+document.getElementById(r2_room_number)-->
            </div>
        </div>
</div>
...
%>
<script>
document.getElementById("r2_room_number").value="<%=roomnum%>";
