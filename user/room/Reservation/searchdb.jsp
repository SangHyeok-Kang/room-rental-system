...
<% 
	...
	stb.append("select *,(6371*acos(cos(radians(").append(entX).append("))*cos(radians(").append("x").append("))*cos(radians(").append("y").append(")-radians(").append(entY).append("))+sin(radians(").append(entX).append("))*sin(radians(").append("x").append(")))) AS distance  from room where sale = 1");
    
    int[] check = new int[15];

    if  (options[0].equals("com"))stb.append(" and computer = 1");
    for (int i = 0; i < options.length; i++) {
                    if (options[i].equals("micro")) {
                        stb.append(" and microwave  =  1");
                    } if (options[i].equals("airfy")) {
                       stb.append(" and airfryer  =  1");
                    }if (options[i].equals("tv")) {
                        stb.append(" and tv  =  1");
                    }if (options[i].equals("laund")) {
                      stb.append(" and laundry  =  1");
                    }if (options[i].equals("aircon")) {
                        stb.append(" and airconditioner  =  1");
                    }if (options[i].equals("refri")) {
                      stb.append(" and refrigerator  =  1");
                    }if (options[i].equals("wifi")) {
                        stb.append(" and wifi  =  1");
                    }if (options[i].equals("tableware")) {
                       stb.append(" and tableware  =  1");
                    }if (options[i].equals("kitchen")) {
                       stb.append(" and kitchentools  =  1");
                    }if (options[i].equals("tow")) {
                       stb.append(" and towel  =  1");
                    }if (options[i].equals("bed")) {
                       stb.append(" and bed  =  1");
                    } if (options[i].equals("desk")) {
                      stb.append(" and desk  =  1");
                    } if (options[i].equals("drawer")) {
                     stb.append(" and drawer  =  1");
                    }
                    
    } 
    stb.append(" HAVING distance <= 10");
...
function init(){
		opener.searchdbCallBack("<%=juso_list%>","<%=juso_x%>","<%=juso_y%>","<%=money%>","<%=start_time%>","<%=end_time%>","<%=city%>","<%=gu%>","<%=ro%>","<%=d_address%>","<%=room_num%>","<%=run%>");
		window.close();
} 
